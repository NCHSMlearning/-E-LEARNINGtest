...
class Database {
    ...
    async updateProfile(updates) {
        ...
        // === FEE BALANCE FUNCTIONS ===
    
    // Get student fee summary
    async getFeeSummary() {
        try {
            console.log('💰 Getting fee summary for student:', this.currentUserId);
            ...
            const { data: feeConfig, error: feeError } = await this.supabase
                .from('fee_structure')
                .select('amount')
            ...
            return {
                ...
                isCleared: outstanding <= 0,
                block: block,
                program: student.program
            };
            
        } catch (error) {
            console.error('Failed to get fee summary:', error);
            return null;
        }
    }
    ...
    // Get approved registered units for exam card
    async getApprovedUnits() {
        try {
            console.log('📇 Getting approved registered units...');
            ...
            console.error('Failed to get approved units:', error);
            return [];
        }
    }
    
    // Get exam eligibility (fee cleared AND has approved units)
    async getExamEligibility() {
        try {
            ...
            const isFeeCleared = feeSummary ? feeSummary.outstanding <= 0 : false;
            
            // Check approved units
            const approvedUnits = await this.getApprovedUnits();
            const hasApprovedUnits = approvedUnits.length > 0;
            
            // Get pending units count
            const { data: pendingUnits, error: pendingError } = await this.supabase
                .from('student_unit_registrations')
                .select('id')
                .eq('student_id', this.currentUserId)
                .eq('status', 'pending');
            ...
            return {
                isEligible: isFeeCleared && hasApprovedUnits,
                isFeeCleared: isFeeCleared,
                hasApprovedUnits: hasApprovedUnits,
                approvedUnitsCount: approvedUnits.length,
                pendingUnitsCount: pendingUnits?.length || 0,
                outstandingBalance: feeSummary?.outstanding || 0,
                message: this.getEligibilityMessage(isFeeCleared, hasApprovedUnits, feeSummary?.outstanding)
            };
            
        } catch (error) {
            console.error('Failed to get exam eligibility:', error);
            return {
                isEligible: false,
                isFeeCleared: false,
                hasApprovedUnits: false,
                approvedUnitsCount: 0,
                pendingUnitsCount: 0,
                outstandingBalance: 0,
                message: 'Unable to determine eligibility. Please contact admin.'
            };
        }
    }
    
    // Get eligibility message
    getEligibilityMessage(isFeeCleared, hasApprovedUnits, outstandingBalance) {
        if (!isFeeCleared && !hasApprovedUnits) {
            return `❌ Fee balance of KES ${outstandingBalance?.toLocaleString()} outstanding AND no approved units. Please clear fees and register units.`;
        } else if (!isFeeCleared) {
            return `❌ Fee balance of KES ${outstandingBalance?.toLocaleString()} outstanding. Please clear fees to be eligible for exams.`;
        } else if (!hasApprovedUnits) {
            return `❌ No approved unit registrations found. Please register units through the Learning Hub and wait for admin approval.`;
        } else {
            return `✅ You are cleared to sit for examinations.`;
        }
    }
    
    // Get exam card data (complete for printing)
    async getExamCardData() {
        try {
            console.log('📇 Generating exam card data...');
            
            const student = this.currentUserProfile;
            const feeSummary = await this.getFeeSummary();
            const approvedUnits = await this.getApprovedUnits();
            const eligibility = await this.getExamEligibility();
            
            return {
                student: {
                    name: student?.full_name || 'N/A',
                    studentId: student?.student_id || student?.user_id?.substring(0, 8) || 'N/A',
                    program: student?.program || 'N/A',
                    intakeYear: student?.intake_year || 'N/A',
                    block: student?.block || 'Introductory',
                    email: student?.email || 'N/A'
                },
                feeSummary: feeSummary,
                approvedUnits: approvedUnits,
                eligibility: eligibility,
                examPeriod: this.getExamPeriod(),
            ...
            console.error('Failed to get exam card data:', error);
            return null;
        }
    }
    ...
    // Get registered units count for dashboard
    async getRegisteredUnitsCount() {
        try {
            const { data: registrations, error } = await this.supabase
                .from('student_unit_registrations')
                .select('id')
            ...
            console.error('Failed to get registered units count:', error);
            return 0;
        }
    }
