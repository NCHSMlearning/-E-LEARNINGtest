// js/ui.js - COMPLETE FIXED VERSION WITH FEE BALANCE & EXAM CARD
class UIModule {
    constructor() {
        ...
        // Define valid tabs - ADDED fee-balance AND exam-card
        this.validTabs = [
            'dashboard', 
            'profile', 
            'calendar', 
            ...
            'unit-registration',
            'learning-hub',
            'fee-balance',      // ← NEW: Fee Balance tab
            'exam-card'         // ← NEW: Exam Card tab
        ];
        
        // Define clean URL paths - ADDED fee-balance AND exam-card
        this.tabPaths = {
            'dashboard': '/',
            'profile': '/profile', 
            'calendar': '/calendar',
            ...
            'fee-balance': '/fee-balance',     // ← NEW
            'exam-card': '/exam-card'           // ← NEW
        };
        ...
        // Tab display names - ADDED fee-balance AND exam-card
        this.tabNames = {
            'dashboard': 'Dashboard',
            'profile': 'Profile',
    ...
    setupProfileDropdown() {
        ...
        setTimeout(() => {
            // Remove old problematic dropdown if exists
            const oldDropdown = document.querySelector('.dropdown-menu');
            if (oldDropdown) {
                oldDropdown.remove();
            }
            
            // Create NEW simple dropdown
            this.createSimpleDropdown();
            
            // Setup trigger with simple click handler
    ...
    loadTabModule(tabId) {
        ...
        setTimeout(() => {
            switch(tabId) {
                ...
                // =====================================================
                // NEW: Fee Balance and Exam Card Modules
                // =====================================================
                    
                case 'fee-balance':
                    console.log('💰 Loading Fee Balance module...');
                    if (typeof initFeeBalance === 'function') {
                        initFeeBalance();
                    } else {
                        console.warn('⚠️ initFeeBalance not found');
                        this.showToast('Fee Balance module loading...', 'info', 2000);
                    }
                    break;
                ...
                case 'exam-card':
                    ...
                    if (typeof initExamCard === 'function') {
                        ...
                        this.showToast('Exam Card module loading...', 'info', 2000);
                    }
                    break;
    ...
    refreshDashboard() {
        ...
        // Also refresh fee data and exam card if modules exist
        if (typeof loadStudentFeeData === 'function') {
            loadStudentFeeData();
        }
        if (typeof loadExamCard === 'function') {
            loadExamCard();
        }
    }
    ...
    setupTabChangeListener() {
        window.addEventListener('tabChanged', (e) => {
            console.log(`🔄 Tab changed from ${e.detail.previousTab} to ${e.detail.tabId}`);
        });
    }
...
// Ensure tab persistence on page refresh
window.addEventListener('load', function() {
    console.log('📱 Page fully loaded - Ensuring URL persistence');
    
    setTimeout(() => {
        if (window.ui) {
            const path = window.ui.getCurrentPath();
            console.log('📍 Initial path on load:', path);
            
            const currentTabFromPath = window.ui.pathToTab[path];
            if (currentTabFromPath && currentTabFromPath !== window.ui.currentTab) {
                console.log(`🔄 URL suggests ${currentTabFromPath}, updating UI`);
                window.ui.showTab(currentTabFromPath, true);
            }
        }
    }, 500);
});
